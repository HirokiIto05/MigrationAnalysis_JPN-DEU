...
#| fig-width: 12
df_jp |>
  generate_df_main(total, 0.90) |>
  create_scatter(lag_ln_total)
```

```{r}
df_jp |> 
  generate_df_main(total, cutoff = 0.9) |>  
  create_lm("ln_change_rate_total", "lag_ln_total") |>
  create_model_summary("top90")
```
...
```{r}
df_jp |>
  generate_df_main(total, cutoff = 0.9, lag_i = 2) |> 
  dplyr::filter(
    !is.infinite(ln_change_rate_total),
    !is.infinite(lag_ln_total),
    year %in% c(2015, 2017, 2019)
...
#| eval: false
# df_jp_int |> 
#   # generate_df_main(net_internal) |>
#   create_scatter_int(total, y_internal)  
# df_jp_int |>
#   # generate_df_main(net_external) |>
...
```{r}
#| eval: false
estimate_total <- create_lm_robust(df_jp_int |> generate_df_main(net_internal), "ln_change_rate_total", "lag_ln_total_native")
create_model_summary(estimate_total, "Total")

estimate_total <- create_lm_robust(df_jp_int |> generate_df_main(net_external), "ln_change_rate_total", "lag_ln_total_native")
create_model_summary(estimate_total, "Total")
```

```{r}
#| eval: false
estimate_total <- create_lm_robust(df_de_int |> generate_df_main(net_internal), "ln_change_rate_total", "lag_ln_total_native")
create_model_summary(estimate_total, "Total")

estimate_total <- create_lm_robust(df_de_int |> generate_df_main(net_external), "ln_change_rate_total", "lag_ln_total_native")
create_model_summary(estimate_total, "Total")
```
