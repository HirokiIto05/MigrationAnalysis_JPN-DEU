...
```{python}
def generate_urban_rural_pref_japan():
  list_pref_big = [
    "東京都", "神奈川県", "千葉県", "埼玉県", "愛知県", "大阪府", "兵庫県"]

  # list_pref_rural = [
  #   "秋田県", "香川県", "和歌山県", "佐賀県", "山梨県", "福井県", "徳島県", "高知県", "島根県", "鳥取県"]
  list_pref_rural = [
    "大分県", "石川県", "宮崎県", "山形県", "富山県", "秋田県", 
    "香川県", "和歌山県", "佐賀県", "山梨県", "福井県", "徳島県", "高知県", "島根県", "鳥取県"
  ]
  ...
  df = (
    pl.read_excel(
      ...
      .unique()
      .with_columns(
        pl.when(pl.col("prefecture_name").is_in(list_pref_big))
        .then(pl.lit("big"))
        .when(pl.col("prefecture_name").is_in(list_pref_rural))
        .then(pl.lit("rural"))
...
```{python}
def generate_urban_rural_de() :
  
  df_de_total = pl.read_csv(
    here("01_data/intermediate/german/total_master.csv")
  )

  df_de_area_raw = pl.read_excel(
    source = here("01_data/raw/german/area/area.xlsx")
  )
  
  df_de_area = (
    df_de_area_raw
    .select(
      [df_de_area_raw.columns[0],
      df_de_area_raw.columns[2]] 
      )
    )

  df_de_area.columns = ["county_id", "area"]

  df_de_area = (
    df_de_area
    .with_columns([
      pl.col("area").cast(pl.Float64, strict=False),
      pl.col("county_id").cast(pl.Int64, strict=False)]
      )
  ...
  df_output = (
    ...
    .join(df_de_area, on="county_id", how="left")
    .rename(
      {"population" : "total"}
    )
    .with_columns(
      [(pl.col("total") / pl.col("area")).alias("density")]
    )
    .with_columns(
      pl.when(pl.col("total") >= 500_000).then(pl.lit("big"))
      .when((pl.col("density") >= 150) & (pl.col("total") >= 100_000)).then(pl.lit("urban"))
      .otherwise( pl.lit("rural"))
      .alias("class_urban")
    )
    .drop(["density", "total", "area"]) 
  )
...
```{python}
# Most frequent
# Count class by nuts2_code
df_city_class_de = (
  df_class_urban_de
  .filter(pl.col("year") == 2019)
  .group_by(["nuts_code2", "class_urban", "city_name"])
  .agg(pl.count().alias("n"))
  # Extract the most frequent class for each nuts2_code
  .sort("n", descending=True)
  .group_by("nuts_code2", "city_name")
  .head(1)
  .drop("n")
  # .filter(pl.col("city_name") != "Saarland")
  )
```
...
# df_city_class_de = (
#   df_class_urban_de
#   .join(df_de_foreign_19, on=["county_id", "year"], how="left")
#   .filter(pl.col("year") == 2019)
#   .group_by(["nuts_code2", "class_urban", "city_name"])
#   .agg(pl.col("total_value").mean().alias("sum_value"))
#   # Extract the most frequent class for each nuts2_code
#   .filter(pl.col("city_name") != "Saarland")
#   .sort(["nuts_code2", "sum_value"], descending=True)
#   .group_by("nuts_code2", "city_name")
#   .head(1)
#   )
```
...
```{python}
df_edu_de = (
  df_edu_de_raw.join(df_city_class_de, on="city_name", how="left")
  )
```
...
NUTS3(391)で分類されている rural, big, urbanを無理やり、NUTS2(37)に分類し直しているため、urbanの中にbig市区町村が含まれ、ruralの中にurban市区町村が含まれていることに注意。
...
- 人口シェア法
  ...
  NUTS2 --->>> big (人口シェアがbigが最も大きいため)



ただし、どれも微妙。

NUTS3のDegree of Urbanisation分類は
...
市区町村ごとに big/urban/rural の分類を行い、その後、nuts2_codeごとに最頻値を抽出。
...
(
  GT(df_base_tbl)
  .tab_header(
    title = "Number of cities by degree of urbanization",
    subtitle = "Japan and Germany"
  )
  .tab_spanner(
    label = "Germany",
    columns = ["type_urban_rural_de", "n_de"]
  )
  .tab_spanner(
    label = "Japan",
    columns = ["type_urban_rural", "n_jp"]
  )
  .cols_label(
    type_urban_rural_de = html("City Size"),
    n_de = html("n"),
    type_urban_rural = html("City Size"),
    n_jp = html("n")
  )
  )
...
df_city_size_de = (
  pl.read_excel(
  here("01_data/intermediate/german/de_city_size.xlsx")
  )
...
# Combine both
df_plot_both = pl.concat([df_plot_de, df_plot_jp], how="vertical")
