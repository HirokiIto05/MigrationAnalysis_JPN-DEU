...
df_edu_jp_analysis = (
  df_edu_jp
  .join(df_class_urban_jp_pref, on="prefecture_name", how="left")
)
```

```{python}
df_class_urban_de = df_class_urban_de_nuts2.join(df_cores, on=["nuts_code2"], how = "left")
```

```{python}
print("Unique values in 'region_type':", df_class_urban_de_nuts2["region_type"].unique())
```

```{python}
df_edu_de = (
  df_edu_de_raw.join(df_class_urban_de_nuts2, on="city_name", how="left")
  )
```
